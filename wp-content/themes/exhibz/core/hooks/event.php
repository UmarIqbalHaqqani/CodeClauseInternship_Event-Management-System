...
add_action('template_redirect',function(){
	...
	if ( is_array($ticket_variation) && count($ticket_variation) === 1 ) {
		...
		add_action('etn_before_add_to_cart_widget_block',function(){
			...
			$stripe_class = ( $etn_sells_engine_stripe == 'checked' ) ?  'stripe_active' : 'woo_active';

			?>
			<div class="sinlge-event-registration <?php echo $stripe_class; ?>">
				<div class="container">
			<?php
		});
...
function exhibz_event_attendee_list(){
	...
	$attendee_show = exhibz_option("attendee_show");
	$attendee_length = exhibz_option("attendee_length");
	if( $attendee_show=="yes" ){
	if( $attendee_enabled ){
        ...
        <?php        if ( is_array( $event_attendees ) && !empty( $event_attendees ) ) {
            ...
            foreach ( $event_attendees as $attendee ) {
                ...
                if ( !empty( $attendee_email ) ) {
					...
					$default_avatar_url = plugins_url('wp-event-solution/assets/images/avatar.jpg');
                ...
                <div class="etn-col-lg-4 etn-col-md-6">
                    <div class="etn-event-attendee-single">
                            <div class="etn-attendee etn-attendee-avatar-wrap mb-2">
                                <img alt="event-image" class="etn-attendee-avatar img-responsive" src="<?php echo esc_url( $attendee_avatar ); ?>" />
                            </div>
