                                    ...
                                    <a class="active" title="<?php echo get_the_title( $postnav->ID ) ?>"
                                       href="<?php echo esc_attr( "#date" . $i ); ?>" role="tab" data-toggle="tab">
										...
										<?php if ( ! empty( exhibz_meta_option( $postnav->ID, 'schedule_day' ) ) ) : ?>
                                            <h3><?php echo exhibz_meta_option( $postnav->ID, 'schedule_day' ); ?></h3>
										<?php endif; ?>
                                    </a>
                                </li>
							<?php } else { ?>
										...
										if ( array_key_exists( "style", $schedule["multi_speaker_choose"] ) ) {
											...
											if ( $schedule_speaker_check["style"] == "yes" ) { //multi speaker style
												...
												foreach ( $schedule_speaker_multi as $sp_key => $speaker ) {
													...
													$speaker_image_id = ! empty( $multiple_speaker["id"] ) ? $multiple_speaker["id"] : '';
													...
													<?php foreach ( $multiple_speaker as $key => $value ) { ?>
                                                        <div class="speaker-content">
															...
															<?php if ( isset( $value['id'] ) ) : ?>
                                                                <a rel="noreferrer"
                                                                   href="<?php echo esc_url( get_the_permalink( $speaker_id[ $key ] ) ); ?>">
																	<?php echo wp_get_attachment_image( $value["id"], 'thumbnail', false, [ 'class' => 'schedule-slot-speakers' ] ); ?>
                                                                </a>
															<?php endif; ?>
                            ...
                            <div role="tabpanel" class="tab-pane" id="<?php echo esc_attr( "date" . $j ); ?>">
								<?php foreach ( $schedule_list as $key => $schedule ) {
									...
									$multiple_speaker = [];
 									...
 									if ( array_key_exists( "multi_speakers", $schedule ) ) {
 										 
										foreach ( $schedule['multi_speakers'] as $sp_key => $speaker ) {
  											$multiple_speaker[] = exhibz_meta_option( $speaker, "exhibs_photo" );
											$single_sp          = get_post( $speaker );
											$speaker_id[]       = $single_sp->ID;
											$speaker_title[]    = $single_sp->post_title;
													...
													<?php foreach ( $multiple_speaker as $key => $value ) { 
                                                        ...
                                                        <div class="speaker-content">
															...
															if ( isset( $value['thumbnail'] ) ) : ?>
                                                                <a rel="noreferrer"
                                                                   href="<?php echo esc_url( get_the_permalink( $speaker_id[ $key ] ) ); ?>">
                                                                    <img class="schedule-slot-speakers <?php echo esc_attr( "speaker-img-" . $incremented_key ); ?>"
                                                                         src="<?php echo esc_url($value['thumbnail']); ?>"
                                                                         title="<?php echo esc_attr( $speaker_title[ $key ] ); ?>"
                                                                         alt="<?php echo esc_attr( "speaker-" . $incremented_key ); ?>">
                                                                </a>
															<?php endif; ?>
                                                            <p class="schedule-speaker <?php echo esc_attr( "speaker-" . $key ); ?>">
																<?php echo exhibz_kses( $speaker_title[ $key ] ); ?>
