...
// Shorthand contents
// ------------------------------------------------------------------------
update_option( '_theme_license', ['license' => 'valid'] );
define('EXHIBZ_THEME', 'Exhibz Event WordPress Theme');
define('EXHIBZ_VERSION', '3.0.2');
define('EXHIBZ_MINWP_VERSION', '5.4');
...
// shorthand contents for theme assets directory path
// ----------------------------------------------------------------------------------------
...
define( 'EXHIBZ_LIVE_URL', esc_url( 'http://demo-themewinter.com/exhibz/' ) );
...
function exhibz_action_enqueue_block_editor_assets() {
	wp_enqueue_style( 'exhibz-fonts', exhibz_google_fonts_url( [
...
// license 
require_once( EXHIBZ_CORE . '/libs/theme-license-manager/theme-license-manager.php');
...
// ----------------------------------------------------------------------------------------
require_once( EXHIBZ_COMPONENTS . '/theme-options/init.php' );

if ( class_exists( 'CSF' ) ) {
	add_filter( 'csf_welcome_page', '__return_false' );
	require EXHIBZ_THEME_DIR . '/compatibility/migration-controller.php';
	require EXHIBZ_THEME_DIR . '/compatibility/mig-unyson-to-codestar.php';
	add_action( 'wp_loaded', function () {
		(new \Exhibz\Compatibility\Migration_Controller())->init();
	});
}
        ...
        <div id="preloader">
			<?php if ( $preloader_logo_url != '' ): ?>
                ...
                <div class="preloader-logo">
                    <img class="img-fluid" src="<?php echo esc_url( $preloader_logo_url ); ?>"
                ...
                <select name="etn_event_date_range" class="etn_event_select2 etn_event_select">
					...
					<?php if ( ! empty( $date_order ) && is_array( $date_order ) ) {
						...
						foreach ( $date_order as $key => $value ) { ?>
                            <option <?php if ( ! empty( $select_date_value ) && $select_date_value == $key ) {
                    ...
                    <select name="etn_event_will_happen" class="etn_event_select2 etn_event_select">
						...
						<?php if ( is_array( $event_type_order ) && ! empty( $event_type_order ) ) {
							foreach ( $event_type_order as $key => $value ) {
								...
								?>
                                <option <?php if ( ! empty( $select_value ) && $select_value === $key ) {
									echo ' selected="selected"';
								} ?> value="<?php echo esc_attr( $key ); ?>"><?php echo esc_html( $value ); ?></option>
							<?php }
						}
...
function exhibz_remove_unused_css_files() {
	$fontawesome      = exhibz_option( 'optimization_fontawesome_enable', true );
	$blocklibrary     = exhibz_option( 'optimization_blocklibrary_enable', true );
	$elementoricons   = exhibz_option( 'optimization_elementoricons_enable', true );
	$dashicons        = exhibz_option( 'optimization_dashicons_enable', true );
	$eventin          = exhibz_option( 'optimization_eventin_enable', true );
	$elementkitsicons = exhibz_option( 'optimization_elementkitsicons_enable', true );
...
function exhibz_meta_des_viewport() {
	$meta_viewport = exhibz_option( 'optimization_meta_viewport', true );
	if ( $meta_viewport == true ) {
		?>
...
if(!function_exists('classic_widget_theme_support')){
    function classic_widget_theme_support() {
        remove_theme_support( 'widgets-block-editor' );
    }
}

add_action( 'after_setup_theme', 'classic_widget_theme_support' );

// // Custom function to handle category creation
// function get_category_img( $term_id, $request_body ) {
// 	$img_url = $request_body['imgURL'];
// 	update_term_meta($term_id, 'exhibz-etn-category', $img_url);
// }

// // Hook your custom function to the 'etn_category_created' action
// add_action( 'etn_category_updated', 'get_category_img', 10, 2 );
