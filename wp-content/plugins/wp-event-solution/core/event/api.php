		...
		if ( ! empty( $ticket_details ) ) {
			foreach ( $ticket_details as $key => $value ) {
				$prices[$key] = [
					...
					'etn_min_ticket'  => $value['etn_min_ticket'],
					'etn_max_ticket'  => $value['etn_max_ticket'],
					'ticket_price'    => (float) $value['etn_ticket_price'],
					'ticket_quantity' => $value['etn_avaiilable_tickets'] ? (int) $value['etn_avaiilable_tickets'] : $ticket_quantity,
