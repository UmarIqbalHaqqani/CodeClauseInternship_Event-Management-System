...
class Api_Booking extends Api {
    ...
    protected function booking_appointment ($data, $id) {
        ...
        $appointment_value  = Api_Appointment::instance()->prepare_item( $meeting );
        $all_seats          = (array) $meeting->get_seats();
        $total_price        = 0;

        
        if( ! $id ) {
            //check if the seats is available
            if( $all_seats && $seats && 'google-meet' != $location_type ) {

                $seats_plan = ( isset( $appointment_value['seat_plan'] ) && !empty($appointment_value['seat_plan']) ) ? $appointment_value['seat_plan'] : [];

                foreach( $seats as $seat ) {
        
                    if( in_array( $seat, $all_seats ) ){
                        foreach( $seats_plan as $seat_plan ) {
                            if( $seat_plan['id'] == $seat ) {
                                $total_price += intval( $seat_plan['price'] );
                            }
                        }
                ...
                $price = ( isset( $appointment_value['price'] ) && !empty($appointment_value['price']) ) ? $appointment_value['price'][0] : [];
                $total_price = intval( $price['ticket_quantity'] ) * intval( $price['ticket_price'] );

            }

            if ( $total_price != $order_total && 'google-meet' != $location_type ) {
                $data = [
                    'status_code' => 403,
                    'success'     => 0,
                    'message'     => 'Pricing is not matched',
                ];
                return new WP_HTTP_Response( $data, 403 );
            }    

            if ( $staff->get_full_name() != $appointment_value['staff'][0]['full_name'] ){
                $data = [
                    'status_code' => 403,
                    'success'     => 0,
                    'message'     => 'Administrator name not matched',
                ];
                return new WP_HTTP_Response( $data, 403 );
            }
            
            
            if ( $location != $appointment_value['locations'][0]['location'] ) {
                $data = [
                    'status_code' => 403,
                    'success'     => 0,
                    'message'     => 'Location type not matched',
                ];
                return new WP_HTTP_Response( $data, 403 );
            }
